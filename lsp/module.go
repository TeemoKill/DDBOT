...
func (l *Lsp) sendGroupMessage(groupCode int64, msg *message.SendingMessage, recovered ...bool) (res *message.GroupMessage) {
	...
	sendGroupMessageRetry := func(
		groupCode int64, msg *message.SendingMessage, retryLimit int, flags ...bool,
	) (resp *message.GroupMessage) {
		var retry = 0
		for {
			resp = bot.Instance.SendGroupMessage(groupCode, msg, flags...)
	...
	res = sendGroupMessageRetry(groupCode, msg, 3, cfg.GetFramMessage())
	if res == nil {
		res = &message.GroupMessage{Id: -1, Elements: msg.Elements}
	}
	return res
